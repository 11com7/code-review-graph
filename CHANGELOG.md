...
- **Windows: `embed_graph_tool` hangs on cold model load** (11com7): when the background `crg-prewarm` thread was still loading PyTorch / sentence-transformers DLLs and `embed_graph_tool` was called concurrently, `asyncio.to_thread` needed to create a new thread-pool worker. On Windows, creating a thread calls `DllMain(DLL_THREAD_ATTACH)` for every loaded DLL, which requires the OS DLL Loader Lock — the same lock held by the pre-warm thread while loading DLLs. The new worker stalled for 10–60 s (or indefinitely) waiting for the lock, causing MCP client timeouts. Fixed by adding a `_prewarm_done` `threading.Event` that `crg-prewarm` sets in its `finally` block. `embed_graph_tool` now polls that event with `await asyncio.sleep(0.5)` on Windows before calling `asyncio.to_thread`, keeping the event loop responsive and ensuring worker creation only happens after DLL loading is complete. See: #46, #136
