...
def _handle_init(args: argparse.Namespace) -> None:
    ...
    if not skip_skills:
        # Claude Code skills (.claude/skills/) are read by Claude Code and Qoder.
        if target in ("claude", "claude-code", "qoder", "all"):
            skills_dir = generate_skills(repo_root)
            print(f"Generated Claude Code skills in {skills_dir}")
        elif target not in ("gemini-cli",):
            ...
            print(
                ...
                f"skills are read only by Claude Code / Qoder."
            )
